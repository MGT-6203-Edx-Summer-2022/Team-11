...
#-------------------------------------------------------------------------------
# Load  data
df <- read.csv('Macro_SP_QoQ.csv', header = TRUE, fileEncoding="UTF-8-BOM")
...
plot(final_leaps, scale = "bic")
# summary(final_leaps)
# same model as forward stepwise model
```
...
plot(final_leaps, scale = "bic")
# summary(final_leaps)
# same model as forward stepwise regression model
```
...
fit_lasso_cv_sol0 = cv.glmnet(x=as.matrix(QoQ_scaled[,-22]),y=as.matrix(QoQ_scaled$SP500),
                    alpha=0.775,nfolds = 5,type.measure="mse",family="gaussian")
...
```{r}
QoQ.lm <- lm(SP500~WTISPLC+GDPC1+PRS85006023+UMCSENT+PERMIT,Macro_SP_QoQ)
summary(QoQ.lm)
```
